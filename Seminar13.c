...
void inserareMuchie(NodPrincipal* graf, int id1, int id2) {
	...
	if (nod1 != NULL && nod2 != NULL) {
		...
		//pentru grafurile orientate avem o singura inserare in lista secundara
		//leg inversa nu se mai face = avem orientare
	}
}
	...
	while (stiva.first) {
		...
		while (vecini) {
			if (vectorNoduriVizitate[vecini->info->info.id - 1] == 0 ) {
				push(&stiva, vecini->info->info.id);//il pun in stiva
				vectorNoduriVizitate[vecini->info->info.id - 1] == 1;//il marcam vizitat
			}
			vecini = vecini->next;
		}
	}
