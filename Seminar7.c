...
struct HashTable {
	Nod** vector; //vector alocat dinamic de pointeri la nod
	int dimensiune;
};
...
void inserareInHT(HashTable ht, Masina masina) {
	if (ht.dimensiune > 0) {
		...
		if (pozitie >= 0 && pozitie < ht.dimensiune) { //daca pozitia calculata este in interval
...
void main() {
	...
	//aici putem transmite prin valoare
	traversareHT(hashT);
	traversareHT(hashT);
}
...
//parcurg vector
//fac dezalocare cand pozitia e diferita de null (de ex in traversare)
//la final facem free la vector